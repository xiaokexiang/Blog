...
- 面试题: 交替打印奇偶数(可以不使用 wait 和 notify 实现)
...
- wait 是所有 Object 拥有的方法,sleep 是 Thread 的方法
- wait 会释放锁,sleep 会占有锁
- wait 需要被 wakeup,sleep 不需要
- 使用 wait 必须要有 synchronize,sleep 不需要
